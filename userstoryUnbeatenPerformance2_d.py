...
#find games with the club name
    allGames=graph.run("match(n:Game)<--(s:Season {name:'Year"+year+"'}) where n.name contains '"+club+"' return n.name,n.winner")
    listOfGames=[]
...
def findUnbeatenPerformanceOfAClub():
    ...
    for season in allSeasons:
        ...
        if(gamesCount <= 0):
            print("\nThe season "+season["s.name"]+" did not have unbeaten performance by the club "+club)                
